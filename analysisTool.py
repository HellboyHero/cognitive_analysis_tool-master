...
# Use tkinter for Windows/Linux, PyObjC for macOS
...
class WindowsInterface:
    def __init__(self, master):
        self.master = master
        self.master.title("Cognitive Workload Analysis Tool")
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text="Recording Folder:").pack(anchor=tk.W, pady=(0, 5))
        self.recording_folder_var = tk.StringVar(value=self.settings['recordingFolder'] or "Not selected")
        ttk.Entry(main_frame, textvariable=self.recording_folder_var, state='readonly').pack(fill=tk.X, pady=(0, 10))
        ttk.Button(main_frame, text="Select Recording Folder", command=self.select_recording_folder).pack(anchor=tk.W)
        
        # Luminance folder selection
        ttk.Label(main_frame, text="Luminance Data Folder:").pack(anchor=tk.W, pady=(10, 5))
        self.lux_folder_var = tk.StringVar(value=self.settings['luxFolder'] or "Not selected")
        ttk.Entry(main_frame, textvariable=self.lux_folder_var, state='readonly').pack(fill=tk.X, pady=(0, 10))
        ttk.Button(main_frame, text="Select Luminance Data", command=self.select_lux_folder).pack(anchor=tk.W)
        
        # Export folder selection
        ttk.Label(main_frame, text="Export Folder:").pack(anchor=tk.W, pady=(10, 5))
        self.export_folder_var = tk.StringVar(value=self.settings['exportFolder'] or "Not selected")
        ttk.Entry(main_frame, textvariable=self.export_folder_var, state='readonly').pack(fill=tk.X, pady=(0, 10))
        ttk.Button(main_frame, text="Select Export Folder", command=self.select_export_folder).pack(anchor=tk.W)
        
        # Analysis button
        ttk.Button(main_frame, text="Start Analysis", command=self.start_analysis).pack(pady=20)
        
        # Status bar
        self.status_var = tk.StringVar(value="Ready")
        ttk.Label(main_frame, textvariable=self.status_var, relief=tk.SUNKEN, anchor=tk.W).pack(side=tk.BOTTOM, fill=tk.X, pady=(10, 0))
    
    def select_recording_folder(self):
        folder = filedialog.askdirectory(title="Select Recording Folder")
        if folder:
            self.settings['recordingFolder'] = folder
            self.recording_folder_var.set(folder)
    
    def select_lux_folder(self):
        folder = filedialog.askdirectory(title="Select Luminance Data Folder")
        if folder:
            self.settings['luxFolder'] = folder
            self.lux_folder_var.set(folder)
    
    def select_export_folder(self):
        folder = filedialog.askdirectory(title="Select Export Folder")
        if folder:
            self.settings['exportFolder'] = folder
